...
from module.mobilenetv2 import mobilenetv2
from module.resnet import resnet50
from module.swin import SwinBlock, psp_use_swin, Residual
...
class _PSPModule(nn.Module):
    def __init__(self, in_channels, pool_sizes, norm_layer):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_channels + (out_channels * (len(pool_sizes)+1)), out_channels, kernel_size=3, padding=1, bias=False),
            norm_layer(out_channels),
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1)
        )

        # 固定窗口+滑动窗口  : 1.仅仅是单独加一个 56 x 56的
        # dim:表示通道数，heads:表示transformer的头数，head_dim
        ...
        # regular_block 是固定窗口，shifted_block 是滑动窗口
        self.regular_block = psp_use_swin(dim=self.dim, heads=self.heads, head_dim=self.head_dim,
                                          shifted=False, window_size=self.window_size,
                                          relative_pos_embedding=self.relative_pos_embedding)
        self.shifted_block = psp_use_swin(dim=self.dim, heads=self.heads, head_dim=self.head_dim,
                                          shifted=True, window_size=self.window_size,
                                          relative_pos_embedding=self.relative_pos_embedding)
    ...
    def forward(self, features):
        ...
        # swin    features:[2, 320, 56, 56]
        x_swin_input = features.permute(0, 2, 3, 1)  # [2, 56, 56, 320]
        # print("----",x_swin_input.shape)
        x = nn.Linear(c, 96).cuda()(x_swin_input)  # [2, 56, 56, 96]
        x = self.regular_block(x)
        x = self.shifted_block(x)
        x = nn.Linear(96, 80).cuda()(x).permute(0, 3, 1, 2)
        pyramids.append(x)
