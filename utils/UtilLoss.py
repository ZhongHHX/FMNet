...
class LossHistory():
    ...
    def append_mIoU(self,mIoU):
        ...
        with open(os.path.join(self.save_path, "epoch_miou_"+ str(self.time_str) + ".txt") , 'a') as f:
            f.write(str(mIoU))
            f.write("\n")
        self.miou_plot()
    ...
    def miou_plot(self):
        iters = range(len(self.miou))

        plt.figure()
        plt.plot(iters, self.miou, 'red', linewidth=2, label='val miou')
        #plt.plot(iters, self.val_loss, 'coral', linewidth=2, label='val miou')
        try:
            if len(self.miou) < 25:
                num = 5
            else:
            ...
            plt.plot(iters, scipy.signal.savgol_filter(self.miou, num, 3), 'green', linestyle='--', linewidth=2,
